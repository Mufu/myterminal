...
| --- | --- |
| 新連接 | 開啟新連接對話框，建立工作階段 |
| 輸入字 | 開關終端機下方的多行輸入區，「送出」會把整段內容一次送進作用中的工作階段（用來寫給 Claude / Codex 的長訊息）。**換行怎麼送看工作階段的類型**：PowerShell / WSL / SSH / 自訂命令是一行一個指令，每一行都當成按了 Enter；Claude / Codex / Agent 任務收的是一段多行提示，裡面的換行保留（等於 TUI 的 Ctrl+J），最後才送出 |
| 複製文字 | 複製終端機中選取的文字 |
| 貼上 | 把剪貼簿內容送進作用中的工作階段 |
