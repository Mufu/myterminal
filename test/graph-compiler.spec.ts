...
import type {
  ...
  RunGraphState,
  Timers,
} from '../src/main/workflow/graph-compiler';
...
/** 依照呼叫順序回答的 runner；回傳 null 代表這次永遠不回答 (測逾時用)。*/
class ScriptedRunner implements IAgentRunner {
  ...
  start(task: AgentTask): IAgentRun {
    ...
    // 節點是同步跑到 waitForResult 的，所以要等它掛好 listener 再發事件。
    if (event) queueMicrotask(() => run.emit(event));
    return run;
  }
}

const result = (over: Partial<Extract<AgentEvent, { type: 'result' }>> = {}): AgentEvent => ({
  type: 'result',
  ok: true,
  text: 'ok',
  sessionId: 'cli-1',
...
class FakeSessions implements IWorkflowSessions {
