...
| `cjk-ime.spec.ts` | 5 | 無 | 合成的注音組字事件打進 PowerShell 與 Claude 的 TUI（中間的注音符號不會進 shell、字不重複）、「輸入字」面板的焦點不會被終端機拆走（開啟時、狀態變動時、收起來才還給終端機）、compositionstart 時隱藏輸入區被搬回游標那一格 |
...
合計 27 個問題，23 個已修（每個都有對應的單元或 e2e 測試），4 個只記錄。
...
| 工作階段命名用全域計數器（`PowerShell 1`、`WSL 2`、`PowerShell 3`） | 改成每種型別各自編號（`PowerShell 1`、`WSL 1`、`PowerShell 2`）；id 仍然是全域唯一，取了名字的不佔號碼 |

只記錄、未改：

- Codex 不回報金額（只有 token 數），所以用量上限對 Codex 節點無效。用量欄本身已經改成
  沒有金額時寫 token 數（`11.9k tokens`），至少看得到用掉多少。
